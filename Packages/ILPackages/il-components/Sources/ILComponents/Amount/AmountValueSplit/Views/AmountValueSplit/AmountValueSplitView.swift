...
private extension AmountValueSplitView {
	...
	func amountView () -> some View {
		HStack {
			...
			AmountView(vm.formattedAmount)
				.amountViewAppearance(vm.amountViewAppearance)
		}
		.listRowInsets(.init())
		.padding(8)
	}
	...
	func exactSumView () -> some View {
		Text(vm.usersSplitValue.doubleExactValue, format: .currency(code: vm.currency.code))
	}
